...
def generate_random_customers(num_customers,missing_rows):
    customers = []
    ...
    for i in range(num_customers):
        ...
        if i < missing_rows:
            # Randomly choose fields to set as None or empty
            fields_to_nullify = random.sample(list(customer.keys()), k=random.randint(1, len(customer)//2))
...
def create_or_append_account(customers,filename):
