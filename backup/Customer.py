...
def create_customer():
    ...
    if not os.path.isfile(account_filename):
        raise FileNotFoundError(f"{account_filename} not found. Ensure the file exists.")

    # Read the number of entries in account.csv
    with open(account_filename, mode='r', newline='') as account_file:
    ...
    filtered_data = [
        {key: row[key] for key in fieldnames} for row in accounts
    ]
